...
| Live-main baseline audited | `96464b8e` (2026-07-18 remediation; agents must fetch and report the exact current SHA at session start) |
| Production-defect remediation (2026-07-18, owner smoke findings — SUPERSEDES the "track COMPLETE" row below) | **Both confirmed defects fixed + deployed.** P0 save outage: `1e8615ce` #1169 (wrapper rejected `clear_fields` → 503 on every save; real-wrapper regression test added; correlation refs on failure surfaces) — deploy **success**. P0 legacy dark shell over the workspace: `e2ba730b` #1170 (WorkspaceShell `mobileChrome`; legacy header public-only; dock removed; single-shell e2e proof) — deploy **success**. Mobile usability: `96464b8e` #1171 (shortcut hints desktop-only; compact save bar; no overflow at 320/360/390 EN+AR). Ledger: TASK-20260718-020…022. **Final acceptance pending owner live evidence** (save + numeric clear succeed; dark top bar + dock gone; one shell; no overlap; shortcuts hidden on mobile; EN + AR RTL). |
