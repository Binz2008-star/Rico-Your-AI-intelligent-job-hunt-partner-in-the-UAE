...
## Lane continuity blocks — reconciled 2026-08-01 at `main` `5a515361`
...
- Production data residuals, read-only characterised and **not** authorised for repair: a legacy three-row production cluster (2 rows wrongly stamped), 3 latent mismatched rows, 21 guest rows holding a `TRUSTED_IDENTITY_FIELD` (19 of them a phone), and 74 guest rows holding authenticated onboarding completion. All pre-date the guards now on `main`; the guards stop new ones
...
- **Pass completed 2026-07-28 (post-`#1426`):** docs-only, branched from `383dcb6c`, covering the same four documents — `PROJECT_STATUS.md`, `TASKS.md`, `ENGINEERING_ROADMAP.md` and `ARCHITECTURE.md`. No new status document, roadmap, proposal or handoff was created. `DECISIONS.md` was **not** modified: `DEC-20260725-001` already states that `store_unavailable` is a different fact from `no_cv_on_file`, and `#1425`/`#1426` implement that decision rather than falsify it. `OPERATING_RULES.md` was read and **not** modified: it carries rules, not state claims, and nothing in `#1424`–`#1426` invalidates one. It touches no `src/`, no tests, no workflows, no migration
- Stale claims this pass removed, each verified against live state first: all four documents recorded baseline `c64aa99`, three merges behind; `PROJECT_STATUS.md` and `ENGINEERING_ROADMAP.md` both instructed that the Journey-1 CV routing characterization was the immediate **unstarted** action after it had merged as `#1424`; `PROJECT_STATUS.md` and `ARCHITECTURE.md` recorded the My Files `files=[]` collapse as live after `#1425` fixed it; the roadmap's Phase 2 "next candidates" keyed its acceptance check to a strict xfail that `#1425` had already converted to a passing contract; and the Arabic/English CV-routing divergence was recorded unqualified after `#1426` narrowed it
- **One evidence-class correction made in this pass:** earlier passes recorded that a `/version` read is "not reproducible from an agent container". This session read `/version` and `/health` directly from the public endpoint, so that limitation is withdrawn. The correction is narrow — it changes who can take the reading, not what the reading proves
- Standing note for whoever reconciles next: this lane has now gone stale four times by carrying a `main` SHA in prose. Every SHA in these documents is a claim that decays; re-read `main` live before trusting any of them. Replace a stale "next action" — never stack a new one beneath it
...
- **L9 — Journey-1 CV read truthfulness (Journey-1 D3).** `CLOSED`. `#1422` merged as `c64aa99158695c78138e15ca4d6dfb57b5c762c7` from approved head `f8073ad28fac798eb44828f98d8d8f7f3c4c63ff`, onto base `3f2805de`. Branch `claude/cv-read-failure-truthful-response-pfub9j`; lease `RELEASED`, write authorization `REVOKED`. It changes one runtime path under `src/`, so a backend deploy was expected and fired: `/version.commit` matched `c64aa99` and `/health` returned 200 / ok with `jooble`, `adzuna` and `jsearch` configured and non-degraded — **owner/browser-verified, and no deliberate CV-store failure smoke was performed, so the changed path itself was never exercised in production.** **Delivered:** the invariant **READ FAILURE != VERIFIED ABSENCE** on the chat CV path. A failed CV grounding or document read no longer becomes "no CV", "no stored CV", unreadable-document blame, upload/re-upload guidance, or `next_action="upload_cv"`, in English and Arabic. A successful empty read is still a genuine absence; `no_readable_content` from a completed read is still a content problem; the exception stays contained. No route, migration, schema or frontend change. The structural statement of the invariant is owned by `ARCHITECTURE.md`. **Rollback:** revert `c64aa99` — additive branching in one module, no migration, no schema, no config, no env var. **Residuals as recorded at `#1422`:** My Files still converts its own failed read into `files=[]`; Arabic and English CV routing diverge at `_looks_like_cv_intent_no_file`; stored-CV/CV-state logic remains spread across `src/rico_chat_api.py`; and Journey-1 D1 (ownership/data consolidation), Journey-1 D2 (pending-artifact activation), Journey-1 D4 (mission/dashboard truth) and Journey-1 D5 (first-useful-result activation) are untouched. **The first two of those have since been addressed — see the continuation below; the rest stand.**

  **Continuation — `#1424` → `#1426`, all merged 2026-07-28.** This lane did not end at `#1422`. Three further PRs landed as one delivery arc: characterize the path, then fix the two defects the characterization exposed. Lease `RELEASED` and write authorization `REVOKED` on all three branches.

  - **`#1424` — Journey-1 CV routing characterization.** Merged as `594a4d3ba88a878b599dd9002515d9212d503094` from branch `tests/journey1-cv-routing-characterization`, onto base `4ce4add8`. **Tests only:** one new file, `tests/unit/test_journey1_cv_routing_characterization.py` (+1026), no runtime path, **no deploy expected**. It discharges `TASK-20260728-001` and satisfies the `ARCHITECTURE.md` requirement that routing and side-effect order be captured on the untouched tree before extraction. It drove the real dispatcher (`_handle_active_user_inner`) with counting proxies at the collaborator edge and spies only at the terminals, so the cascade was observed rather than re-implemented, and two positive controls prove the `== 0` assertions are not vacuous. **Surfaced, and deliberately left unfixed:** My Files rendering an unavailable store as `files=[]` (recorded as the one strict xfail); "analyse my cv please" routing to `_target_role_search_response` — a job search — reading neither the document store nor the CV context; and English/Arabic reaching different job-search terminals for the same intent. Tests at merge: 3954 passed, 2 xfailed across `tests/unit/`. **Rollback:** delete the file.
...
Status: **done** — delivered as `#1424`, merged `594a4d3ba88a878b599dd9002515d9212d503094`
Owner: closed; lease released, write authorization revoked
Branch: `tests/journey1-cv-routing-characterization` (merged)
Issue/PR: `#1424`, merged 2026-07-28

> **Closed. All nine scenarios were characterized against the untouched tree and every
> acceptance criterion below was met.** The file is `tests/unit/test_journey1_cv_routing_characterization.py`,
> reached by the CI directory enumeration with no allowlist entry. Two defects it surfaced were
> fixed in their own scoped PRs afterwards — `#1425` (My Files) and `#1426` (CV-analysis
> routing) — neither in this PR, as the constraints required. The lane record in L9 above
> carries the detail. **The objective and constraints below are preserved as the historical
> statement of the task; they are not a live instruction.**
...
Establish, from evidence, **what the ownership/data consolidation problem actually is**
on production data — its shape, size, blast radius, and the safest available repair
options. Journey-1 D1 is the last Journey-1 item with no factual basis in the control
plane: it is described only as "residuals, including the multi-row production account",
which is not enough to authorize anything.

**This task produces an assessment. It does not produce a repair, and completing it
does not authorize one.**

#### Why this is next

The three merges before it (`#1424`, `#1425`, `#1426`) closed the Journey-1 items that
could be closed with code. D1 cannot: it is a *data* problem, it sits above the `#1389`
HOLD, and the owner ruling that parked `#1389` was made because the data state is not
understood. Understanding it is therefore the next thing that is both useful and safe.

#### Constraints — the binding ones

- **READ-ONLY. No production Neon row mutation is authorized** — no `UPDATE`, `DELETE`,
  `INSERT`, `MERGE`, or DDL against production, under any justification, including a
  "trivially safe" one-row correction and including anything the assessment itself
  concludes is obviously right. **A finding is a finding; repair needs its own owner
  authorization.**
- **No migration file is written, applied, or proposed for execution.** A migration
  *sketch* inside the assessment is allowed only as an illustration of a possible
  option, clearly marked as not-for-execution.
- **No branch deletion, no bulk Neon operation, no schema change.**
...
3. The user-visible consequence of each cluster shape, stated per shape.
4. The specific state behind the `#1389` HOLD, described concretely enough for the
   owner to rule on it.
5. Repair **options** with their risks, each explicitly unauthorized, plus what each
   would need in order to become authorizable.
6. What could not be determined read-only.
...
Status: review — Draft PR `#1481`; merge/deploy forbidden; lane FROZEN awaiting independent review
Owner: Roben; execution delegated to the L7 Windsurf WRITER session (lease transferred from Codex); lane now IDLE
Branch: `agent/control-plane-reconcile-20260801`
Issue/PR: `#1481`
...
- Expected remote HEAD: `eede42cabb6e4a9fc1f901abcf796f34ef7c45ce` (corrective input head for Correction-3)
...
- [x] Remote branch head equals `eede42cabb6e4a9fc1f901abcf796f34ef7c45ce` (Correction-3 input head).
- [x] Worktree is clean.
- [x] Update L7 lane and TASK-20260801-001 from initial 9f5dccfa/b7f9a986 to corrective 5a515361/eede42ca lineage.
- [x] Update #1477 and #1480 continuity blocks to current main 5a515361.
- [x] Reconcile handoff objective, base, input head, production evidence, validation, incomplete work, and next action.
- [x] Preserve 9f5dccfa only where explicitly labelled historical initial evidence.
- [x] Replace invented/future 2026-08-01T12:00:00Z timestamp with real verification timestamp (2026-08-01T10:25:11Z).
- [x] Re-read /version, /health, /proxy/health, live open PRs, and GitHub heads immediately before writing.
- [x] Avoid impossible self-referencing commit claims: record input head eede42ca and state that final PR head must be read live after publication.
- [x] Update PR title to: docs(workspace): reconcile control plane at main@5a515361
- [x] After corrective push, update PR body with base/main 5a515361, corrective input head eede42ca, exact new final head, exact three changed files, CI pending/current state, Draft, no merge/deploy authorization.
- [x] Run git diff --check, exact changed-file scope verification, stale-current-claim scan, documentation consistency check.
- [x] Commit once with: docs(workspace): close corrective reconciliation state
- [x] Re-fetch and confirm remote head still equals eede42ca, then push normally without force.
- [x] Keep PR #1481 Draft. Do not merge.
...
- Base branch: `main`
- Base SHA: `5a5153614dd7e092f93d49abd09c928d32fcb456` (corrected from 9f5dccfa after #1482 merge)
- Expected remote HEAD: `eede42cabb6e4a9fc1f901abcf796f34ef7c45ce` (Correction-3 input head; final published head must be read live from PR #1481 and cannot self-identify inside its own commit)
- Current head SHA: `eede42cabb6e4a9fc1f901abcf796f34ef7c45ce` (Correction-3 input head; the final published SHA is read live from PR #1481)
- Uncommitted changes present: no
- Status: review — Draft PR `#1481`; lane FROZEN awaiting independent review
...
- What is complete: preflight verification, governance file reading, live production evidence re-verification, L7 lane block update, TASK-20260801-001 update, #1477/#1480 continuity block updates, handoff reconciliation, PR title/body updates, validation checks, corrective push, CI observation
- What is incomplete: independent review at exact published head; owner merge authorization (explicitly not granted)
- Known blockers: none at this point
- Validation already run: live GitHub/production reads at 2026-08-01T10:25:11Z — /version returns 5a515361, /health ok, open PRs #1477, #1481, #1483; git diff --check PASS; exact changed-file scope PASS; stale-current-claim scan PASS; documentation consistency PASS
- Validation still required: independent review at exact published head
- Deployment/CI/Neon/Vercel state to check next: none; no deployment or Neon work
