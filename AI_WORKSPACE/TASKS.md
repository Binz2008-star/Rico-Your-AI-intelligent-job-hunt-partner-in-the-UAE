...
### TASK-20260620-032 — Password complexity validation (Issue #640)

Status: done
Owner: Claude
Branch: `feat/password-complexity` (merged → `0ecef2b` via PR #640)
Issue/PR: #640

#### Objective
Enforce password complexity on register and reset-password: ≥8 chars, ≥1 uppercase,
≥1 lowercase, ≥1 digit or symbol. Surface validation errors clearly in the frontend.

#### Backend fix
- `src/schemas/auth.py` — `_check_password_complexity()` applied as `@field_validator` to
  `RegisterRequest.password` and `ResetPasswordRequest.new_password`.
- Returns FastAPI 422 with Pydantic detail array: `msg = "Value error, Password must contain …"`.

#### Frontend fixes (same PR, added during review)
- `apps/web/lib/api.ts` — `register()` + `resetPassword()`: was typing `detail` as `string`,
  so array became `"[object Object]"`. Fixed to use existing `extractDetail()` helper +
  `.replace(/^Value error,\s+/i, "")` to strip Pydantic prefix.
- `apps/web/components/auth/SignupForm.tsx` — `mapSignupError()` was discarding `err.message`
  for 400/422; now passes through `err.message` for those codes.
- Static hint "Min 8 characters · uppercase · lowercase · digit or symbol" added below
  password field in `SignupForm.tsx` and `reset-password/page.tsx`.

#### Verification
- `pytest` CI green. Playwright CI green. Vercel preview green.
- Production smoke PASS (2026-06-20): `weakpass` rejected with clear message ✅,
  `SecurePass1!` accepted, account created, verification email triggered ✅.

---

### TASK-20260620-031 — AI workspace docs update (session 2026-06-19 rollout)

Status: done
Owner: Claude
Branch: docs-only (merged via PR #666)
Issue/PR: #666

#### Objective
Record profile nudge (#665) + Render rollout state in `AI_WORKSPACE/CURRENT_STATE.md`
and `AI_WORKSPACE/TASKS.md` after session completion.

---

### TASK-20260620-030 — render-audit.yml bug fixes

Status: done
Owner: Claude
Branch: `fix/render-audit-bugs` (merged → `58ab189` via PR #667)
Issue/PR: #667

#### Objective
Fix four bugs in `.github/workflows/render-audit.yml` found during run #27866539268.

#### Bugs fixed
1. `limit=200` → `limit=100` (Render API max is 100; was returning HTTP 400).
2. Summary step 11 rewrote stale hardcoded text to read live values from temp files.
3. Step 2 Python block now saves computed `svc_status` string to `/tmp/svc_status.txt`.
4. Step 6 extracts live deploy SHA from `/version` endpoint into `/tmp/deploy_sha.txt`.
5. Step 9 lists cron service names/IDs (in addition to count) in `/tmp/cron_count.txt`.
6. Added `"suspended (idle — wakes on request)"` label for Starter plan idle state.

#### Verification
- CI run triggered post-merge; render-audit.yml run passed.

---

### TASK-20260620-029 — Profile nudge synthetic guard

Status: done
Owner: Claude
Branch: `fix/profile-nudge-synthetic-guard` (merged → `8200811` via PR #665)
Issue/PR: #665

#### Objective
Prevent the `rico-profile-nudge-daily` cron from sending emails to synthetic/internal
addresses. Stamp `profile_nudge_sent_at` so skipped accounts are never retried.

#### Fix
- `src/services/profile_nudge_service.py` — `_is_synthetic_email()` blocks `@ricohunt.com`
  domain and local parts matching: `test`, `test_user`, `dummy`, `demo`, `example`, `seed`,
  `fake`, `user_\d+` (case-insensitive, optional suffix).
- `src/schemas/pipeline.py` — `skipped_synthetic: int = 0` added to `ProfileNudgeResponse`.

#### Verification
- pytest CI green. Cron at `0 5 * * *` running normally.
- Render service ID: `srv-d7vjljrbc2fs73ctkp8g`; health 200 post-merge.
...
- #640/#641 on hold — do not merge until Phase 2 is explicitly scoped. (#640 is done — password complexity merged `0ecef2b`.)
...
4. **#355 Phase 2** — per-user interval settings + Telegram DM notifications. (#641
...
- **Issue #654** — AI Workflow V2 tracker; absorbed timeout/city/profile/routing observations
  ...
  scope confirmation.
- **PR #641** — on hold, awaiting explicit approval. Do not merge. (#640 merged to main as `0ecef2b`.)
