...
Migration is complete. `apps/web/lib/client.ts` has been deleted. All API calls now go through `apps/web/lib/api.ts`.

Remaining service wrappers in `apps/web/services/` (dashboard stats, jobs, applications, settings, health) call `lib/api.ts` directly — do not reintroduce `lib/client.ts`.
