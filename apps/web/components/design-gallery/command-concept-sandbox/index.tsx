...
/**
 * DESIGN GALLERY ENTRY — Command Concept Sandbox
 *
 * Standalone component for /design-gallery only.
 * NOT linked from production navigation. NOT /command. NOT the homepage.
 * ALL data shown is SAMPLE / DEMO — no real AI calls, no real jobs, no real users.
 *
 * Source: design-handoffs/approved-for-gallery/command-concept-sandbox/
 * (owner-reviewed 2026-07-08, "Approved as Design Reference — requires
 * production adaptation"; see REVIEW.md in that folder for the full decision
 * and the required Intent -> Safety Policy -> Agent Runtime -> Persistence ->
 * Confirmation adaptation contract before any production use).
 *
 * All buttons in the child scenes are disabled/reference-only and all data is
 * labeled SIMULATED/DEMO — unchanged from the reviewed package.
 *
 * Stack: framer-motion (existing dependency) + Nocturne CSS tokens. Zero new packages.
 */

import { useState } from "react";
import { motion, AnimatePresence, useReducedMotion } from "framer-motion";
import { ThinkingState } from "./ThinkingState";
import { JobIntelCard } from "./JobIntelCard";
...
const DEMO_LABEL = (
  <div className="fixed top-3 left-1/2 -translate-x-1/2 z-50 pointer-events-none">
    <span className="bg-[rgb(var(--gold)/0.15)] border border-[rgb(var(--gold)/0.30)] text-[rgb(var(--gold))] text-[10px] font-mono uppercase tracking-widest px-3 py-1 rounded-full">
      Design reference — demo data only
    </span>
  </div>
);
