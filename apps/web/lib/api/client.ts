...
const RESOLVED_BASE_URL =
  typeof window === "undefined" ? API_BASE_URL : API_PROXY_BASE_URL;
...
export const authApi = {
  ...
  refreshToken: async (): Promise<void> => {
    throw new Error(
      "Backend auth refresh is not implemented. Use the access_token cookie session.",
    );
  },
};
...
export const applicationsApi = {
  ...
  createManual: async (
    data: ManualApplicationCreateRequest,
  ): Promise<StatusUpdateResponse> => {
...
export const ricoChatApi = {
  ...
  publicChat: async (
    data: RicoPublicChatRequest,
  ): Promise<RicoChatResponse> => {
...
export const linkVerificationApi = {
  verify: async (
    data: LinkVerificationRequest,
  ): Promise<LinkVerificationResponse> => {
    const response = await apiClient.post("/api/v1/links/verify", data);
    return response.data;
  },

  verifyBatch: async (
    urls: string[],
  ): Promise<Record<string, LinkVerificationResponse>> => {
    const response = await apiClient.post("/api/v1/links/verify/batch", {
      urls,
    });
    return response.data;
  },
