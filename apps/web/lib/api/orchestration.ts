...
function applicationStatus(
  node: Application,
): TrajectoryForecast["nodes"][number]["status"] {
  switch (node.status) {
    case "offer":
      return "completed";
    case "rejected":
...
export const orchestrationApi = {
  ...
  getTrajectory: async (): Promise<TrajectoryForecast> => {
    ...
    nodes.push({
      ...
      probability: Math.min(
        0.95,
        0.45 + (profile.completeness_score ?? 0) * 0.5,
      ),
      timeline: "Now",
      status: "current",
    });
    ...
    if (profile.target_roles?.[0]) {
      nodes.push({
        ...
        timeline: applications?.applications.length
          ? "Active pipeline"
          : "Build pipeline",
    ...
    return {
      nodes,
      currentPhase: applications?.applications.length
        ? "active-pipeline"
        : "profile-ready",
    };
  },
  ...
  getSignals: async (): Promise<OpportunitySignal[]> => {
    const jobs = await getJobs(1, 12).catch(() => null);
    if (!jobs?.jobs.length) {
    ...
    return jobs.jobs.map((job) => ({
      ...
      whyItFits:
        ...
        "Rico matched this job against your profile and saved preferences.",
