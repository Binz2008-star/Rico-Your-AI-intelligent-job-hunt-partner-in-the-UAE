...
const COMMAND_LOGIN_HREF = buildAuthHref("/login", "/command-v2");
const COMMAND_SIGNUP_HREF = buildAuthHref("/signup", "/command-v2");

// ─── Rico primitives: thinking + operation state ──────────────────────────────

function ThinkingIndicator() {
    return (
        <div className="flex justify-start" role="status" aria-live="polite" aria-label="Rico is thinking">
            <div className="flex items-center gap-2 px-4 py-3">
                <span className="sr-only">Rico is thinking</span>
                <RicoStatusNode variant="magenta" animation="flicker" />
                <RicoStatusNode variant="cyan" animation="flicker" className="[animation-delay:0.2s]" />
                <RicoStatusNode variant="magenta" animation="flicker" className="[animation-delay:0.4s]" />
            </div>
        </div>
    );
}
...
export default function CommandV2Page() {
        ...
        return (
            <div className="min-h-screen bg-[var(--rico-bg)] flex items-center justify-center">
                <RicoGlassIsland className="flex max-w-lg flex-col items-center gap-4 p-8 text-center">
                    <p className="text-sm font-medium text-[var(--rico-fg-1)]">Session expired.</p>
                    <p className="text-sm text-[var(--rico-fg-3)]">Sign in again to continue chatting with Rico.</p>
                    <RicoButton variant="magenta" onClick={() => router.push(COMMAND_LOGIN_HREF)}>
                        Sign in
                    </RicoButton>
                </RicoGlassIsland>
            </div>
    ...
    return (
        <div className="min-h-screen bg-[var(--rico-bg)] flex flex-col relative overflow-hidden">
            ...
            <header className="relative z-10 flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-[var(--rico-border-subtle)]">
                <Link href="/" className="flex items-center gap-2 text-[var(--rico-fg-1)] font-black text-base sm:text-lg tracking-tight">
                ...
                <nav className="flex items-center gap-2 sm:gap-3">
                    {chatAudience === "authenticated" ? (
                        ...
                        <>
                            <Link href={COMMAND_LOGIN_HREF} className="text-[13px] text-[var(--rico-fg-3)] hover:text-[var(--rico-fg-1)] transition-colors">
                                Sign in
                            </Link>
                            <RicoButton variant="magenta" size="sm" onClick={() => router.push(COMMAND_SIGNUP_HREF)}>
                                Sign up free
                            </RicoButton>
                        </>
                    )}
            ...
            <div className="relative z-10 flex flex-col flex-1 h-[calc(100vh-57px)] sm:h-[calc(100vh-65px)] max-w-3xl w-full mx-auto px-2 sm:px-4">
                ...
                {/* Message list */}
                <div
                    className="flex-1 overflow-y-auto px-2 py-6 space-y-5 pb-32"
                    role="log"
                    aria-live="polite"
                    aria-atomic="false"
                    aria-busy={thinking}
                >
                        ...
                        >
                            ...
                            <div className={`flex flex-col ${m.role === "user" ? "items-end" : "items-start"} max-w-[82%]`}>
                                ...
                                {m.type === "profile_preview" && m.preview && m.filename && editingProfileId !== m.id && (
                                    <div className="mt-3 flex gap-2">
                                        ...
                                        <RicoButton
                                            variant="ghost"
                                            size="sm"
                                            onClick={() => { setEditingProfileId(m.id); setDraftProfile(m.preview!); }}
                                            disabled={thinking}
                                        >
                                            Edit before saving
                                        </RicoButton>
                                ...
                                {m.type === "profile_preview" && editingProfileId === m.id && draftProfile && (
                                    <div className="mt-3 w-full">
                                        <RicoGlassIsland className="p-4 space-y-3">
                                            <p className="text-[11px] font-semibold text-[var(--rico-primary)]">Edit profile</p>
                                            {(
                                                [
                                                    ["name", "Name"],
                                                ...
                                                <label key={field} className="block space-y-0.5">
                                                    <span className="text-[10px] text-[var(--rico-fg-4)]">{label}</span>
                                                    <input
                                            ...
                                            <label className="block space-y-0.5">
                                                <span className="text-[10px] text-[var(--rico-fg-4)]">Skills (comma-separated)</span>
                                                <input
                                                    value={(draftProfile.skills_detected ?? draftProfile.skills ?? []).join(", ")}
                                                    onChange={(e) => {
                                                        const skills = e.target.value.split(",").map((s) => s.trim()).filter(Boolean);
                                                        setDraftProfile((prev) => prev ? { ...prev, skills_detected: skills, skills } : prev);
                                                    }}
                                                    className="w-full rounded-[var(--r-lg)] bg-[rgba(255,255,255,0.04)] border border-[var(--rico-border-soft)] px-3 py-1.5 text-[12px] text-[var(--rico-fg-1)] placeholder:text-[var(--rico-fg-4)] focus:outline-none focus:border-[var(--rico-primary-container)]"
                                                />
                                            </label>
                                            <div className="flex gap-2 pt-1">
                                                ...
                                                <RicoButton
                                                    variant="ghost"
                                                    size="sm"
                                                    onClick={() => { setEditingProfileId(null); setDraftProfile(null); }}
                                                >
                                                    Cancel
                                                </RicoButton>
                                            </div>
                                ...
                                {m.options && m.options.length > 0 && (
                                    <OptionButtons options={m.options} onAction={(p) => sendMessage(p)} />
                                )}
                                ...
                                {/* Source label */}
                                {m.sourceLabel && (
                                    <p className="mt-2 text-[11px] text-[var(--rico-fg-4)]">{m.sourceLabel}</p>
                                )}
                            </div>
                    ...
                    {thinking && (
                        <div className="flex flex-col gap-2">
                            ...
                            {slowHint && (
                                <p className="text-[11px] text-[var(--rico-fg-4)] pl-9 animate-pulse motion-reduce:animate-none" role="status">
                ...
                {/* Floating input bar */}
                <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-[var(--rico-bg)] via-[rgba(0,0,0,0.95)] to-transparent">
                    {uploadError && (
                        <p className="text-[11px] text-[var(--rico-error)] mb-2 text-center" role="alert">{uploadError}</p>
                    )}
                    <div className="flex items-center gap-2">
                        ...
                        <div className="relative flex-1">
                            <RicoCommandInput
                                ...
                                aria-describedby="command-v2-input-hint"
                                className="pr-12"
                            />
                            <button
                                type="button"
                    ...
                    <p id="command-v2-input-hint" className="text-center text-[10px] text-[var(--rico-fg-4)] mt-2 opacity-40">
