  ...
  await page.route("**/api/v1/links/verify**", async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        "job-2": { status: "expired", http_status: 200, error_message: "Expired on Indeed" },
      }),
    });
  });
});
...
test.describe("Opportunity Radar /signals", () => {
  ...
  test("page loads without error state", async ({ page }) => {
    ...
    await expect(page.locator("text=Could not load live signals")).not.toBeVisible();
  });
  ...
  test("cards are in a single column on mobile", async ({ page }) => {
    ...
    await page.waitForSelector("[data-testid='signal-card']", { state: "visible", timeout: 5000 });
  ...
  test("HSE Manager title does not break vertically", async ({ page }) => {
    await page.goto("/signals");
    await page.waitForSelector("[data-testid='signal-card']", { state: "visible", timeout: 5000 });
    const title = page.locator("[data-testid='signal-card']").first().locator("[data-testid='role-title']");
  ...
  test("expired link card does not show View job", async ({ page }) => {
    ...
    const expiredCard = page.locator("[data-testid='signal-card']").nth(1);
    const viewJobBtn = expiredCard.locator("text=View job");
  ...
  test("live card shows View job", async ({ page }) => {
    await page.goto("/signals");
    await page.waitForSelector("[data-testid='signal-card']", { state: "visible", timeout: 5000 });
    const liveCard = page.locator("[data-testid='signal-card']").first();
    const viewJobBtn = liveCard.locator("text=View job");
  ...
  test("Cards / Focus list toggle works", async ({ page }) => {
    ...
    // Find toggle buttons
    const cardsBtn = page.locator("[data-testid='view-cards-btn']");
    const focusBtn = page.locator("[data-testid='view-focus-btn']");
  ...
  test("no infinite API request loop for link verification", async ({ page }) => {
  ...
  test("modal opens on card click and shows actions", async ({ page }) => {
    ...
    // Modal should appear
    const modal = page.locator("[data-testid='signal-modal']");
    ...
    await expect(modal.locator("text=View job")).toBeVisible();
  });
});
