...
def apply_to_job(job: Dict[str, Any], user_id: Optional[str] = None) -> Dict[str, str]:
    """
    Trigger automated application for a job.
    Returns: {"status": str, "message": str, "job_id": str (optional)}
    user_id is required for user-scoped application tracking.
    """
    if not user_id:
    ...
    if "naukrigulf.com" in link:
        return _apply_naukrigulf(job, user_id=user_id)

    if "indeed.com" in link:
        return _apply_indeed(job, user_id=user_id)
...
def _apply_naukrigulf(job: Dict[str, Any], user_id: Optional[str] = None) -> Dict[str, str]:
    try:
...
def _apply_indeed(job: Dict[str, Any], user_id: Optional[str] = None) -> Dict[str, str]:
    try:
