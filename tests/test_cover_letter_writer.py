...
class TestCoverLetterWriter:
    ...
    def test_old_function_still_works_for_backwards_compatibility(self):
        """Test that old generate_cover_letter function still works."""
        job = {
            "title": "HSE Manager",
            "company": "Test Company",
            "location": "Dubai"
        ...
        # Old function should still work with hardcoded identity
        cover_letter = generate_cover_letter(job)
        
        # Should contain the old hardcoded identity
        assert "Roben Edwan" in cover_letter
        assert "Ajman, UAE" in cover_letter
        assert "10+ years in environmental management" in cover_letter
