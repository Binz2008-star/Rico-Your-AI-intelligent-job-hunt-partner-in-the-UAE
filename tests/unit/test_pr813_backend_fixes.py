...
Four test classes, one per smoke-check case:
  ...
  1. TestRuntimeErrorMessages        — BUG #15
     AgentRuntime._build_message must map every KNOWN internal error code to a
     human-readable string so codes never leak to the chat surface. Unknown
     codes and free-text engine errors are not internal enums and keep the
     "Action failed: <error>" operator format (the pre-existing test_607
     contract), so they stay diagnosable.
...
class TestRuntimeErrorMessages:
    ...
    # -- Unknown / arbitrary codes — keep the operator-facing format ----------
    # Shipped contract (matches the pre-existing test_607 contract and #816):
    # only KNOWN internal codes are mapped to user-safe strings. Unknown codes
    # and free-text engine errors are NOT internal enums — they stay in the
    # "Action failed: <error>" operator format so they remain diagnosable.
    # BUG #15's goal (known internal codes never leak) is covered by the
    # MAPPED_CODES tests above.

    def test_unknown_code_keeps_operator_format(self, build_message):
        unknown_code = "some_internal_system_code_xyz"
        result = build_message(action="save", ok=False, data={}, error=unknown_code)
        assert result == f"Action failed: {unknown_code}", (
            f"Unknown (non-enum) error should keep the operator format, got: {result!r}"
        )

    def test_engine_down_free_text_keeps_action_failed_prefix(self, build_message):
        """
        'engine down' is an arbitrary free-text runtime error, not an internal
        enum code. It must keep the "Action failed: <error>" operator format —
        this is the established contract asserted by
        test_607_correctness_fixes::test_apply_failure_path_unchanged.
        """
        result = build_message(action="apply", ok=False, data={}, error="engine down")
        assert result == "Action failed: engine down", (
            f"Free-text engine error must keep the operator format, got: {result!r}"
        )

    def test_none_error_returns_generic_unknown_error(self, build_message):
        result = build_message(action="apply", ok=False, data={}, error=None)
        assert result == "Action failed: unknown error", (
            f"None error should resolve to a generic 'unknown error', got: {result!r}"
        )
...
class TestArabicConversationalRouting:
    ...
    job-search path. The Arabic conversational guard runs after classify_intent
    and short-circuits to _answer_with_ai_fallback when the resolved intent is
    search-like but the message carries no explicit Arabic search command.
    """

    CONVERSATIONAL_ARABIC = [
        "أريد وظيفة في دبي مع راتب 50000 درهم",
        # Declarative job desire, no explicit search command ("ابحث"/"دور عن").
        # NOT "وأبحث عن فرصة" — that contains the search verb أبحث and is a real
        # search request the guard must (correctly) let through.
        "أريد وظيفة كمهندس برمجيات في أبوظبي",
