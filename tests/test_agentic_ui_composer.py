...
# ── Contextual scheduled-search offer (#1249) ────────────────────────────────

class TestScheduledSearchOffer:
    """The "Get these daily" action appears ONLY when the caller established
    it's appropriate (authenticated user, has results, no schedule yet) —
    default compose() output is byte-for-byte unchanged."""

    _RESP = {
        "type": "job_matches",
        "search_query": "Senior HSE Manager",
        "matches": [{"title": "A", "company": "B"}],
    }

    def test_default_output_unchanged(self):
        from src.services.agentic_ui_composer import compose

        r = compose(None, dict(self._RESP))
        ids = [a["id"] for a in r["actions"]]
        assert ids == ["view-jobs", "save-search", "refine-search"]

    def test_offer_appends_schedule_action(self):
        from src.services.agentic_ui_composer import compose

        r = compose(None, dict(self._RESP), offer_scheduled_search=True)
        ids = [a["id"] for a in r["actions"]]
        assert ids == ["view-jobs", "save-search", "schedule-daily-search", "refine-search"]
        sched = next(a for a in r["actions"] if a["id"] == "schedule-daily-search")
        assert sched["kind"] == "chat_continue"
        assert sched["payload"]["message"] == "search daily for Senior HSE Manager jobs"

    def test_offer_payload_parses_as_scheduled_search_create(self):
        """Cross-pin: the button's payload MUST route to the deterministic
        scheduled-search intent, never a plain one-shot search."""
        from src.services.agentic_ui_composer import compose
        from src.services.scheduled_search_service import parse_scheduled_search_command

        r = compose(None, dict(self._RESP), offer_scheduled_search=True)
        sched = next(a for a in r["actions"] if a["id"] == "schedule-daily-search")
        parsed = parse_scheduled_search_command(sched["payload"]["message"])
        assert parsed is not None and parsed["cadence"] == "daily"

    def test_no_matches_never_offers_even_when_eligible(self):
        from src.services.agentic_ui_composer import compose

        r = compose(None, {"type": "job_matches", "search_query": "X", "matches": []},
                    offer_scheduled_search=True)
        ids = [a["id"] for a in r["actions"]]
        assert "schedule-daily-search" not in ids

    def test_other_response_types_never_offer(self):
        from src.services.agentic_ui_composer import compose

        r = compose(None, {"type": "profile_update"}, offer_scheduled_search=True)
        ids = [a["id"] for a in (r or {}).get("actions", [])]
        assert "schedule-daily-search" not in ids
