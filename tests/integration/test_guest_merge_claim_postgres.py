...
(#1070): two accounts claiming the SAME guest concurrently yield exactly one
committed owner (guest-scoped advisory lock + claim-marker CAS inside the data
transaction); the loser copies no guest profile data; a consumed claim cannot
be replayed into a second account; a transient failure rolls the whole claim
back and stays retryable; an unproved claim never reaches the database.

Mock-layer tests cannot prove the lock/CAS serialization — only a real server
runs pg_try_advisory_xact_lock across two connections.
...
def _fresh_guest(tag: str) -> tuple[str, str, str]:
    ...
    from src.api.public_identity import make_guest_proof  # noqa: F401  (proof minted by callers)
...
class TestConcurrentClaim:
    def test_two_accounts_yield_exactly_one_committed_owner(self):
        ...
        ext_a = _ext_of(uid_a)
        ext_b = _ext_of(uid_b)
        proof = make_guest_proof(session_id)
        ...
        def claim(auth_ext: str) -> bool:
            ...
            return merge_public_identity_into_auth(
                public_user_id=public_id, auth_user_id=auth_ext, guest_proof=proof
            )
    ...
    def test_consumed_claim_cannot_be_replayed_into_second_account(self):
        ...
        assert merge_public_identity_into_auth(
            public_user_id=public_id, auth_user_id=_ext_of(uid_a), guest_proof=proof
        ) is True
        # Second account, even WITH the proof: the claim is consumed.
        assert merge_public_identity_into_auth(
            public_user_id=public_id, auth_user_id=_ext_of(uid_b), guest_proof=proof
        ) is False
    ...
    def test_unproved_claim_never_reaches_the_database(self):
        ...
        assert merge_public_identity_into_auth(
            public_user_id=public_id, auth_user_id=_ext_of(uid_a), guest_proof=None
        ) is False
    ...
    def test_transient_failure_rolls_back_claim_and_stays_retryable(self):
        ...
        # Nothing committed: no claim marker, no copied data.
        assert _profile_of(guest_uuid).get("profile_status") != "merged"
        assert "skills" not in _profile_of(uid_a)
